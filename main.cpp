...
bool ExposeFuncs() {
	...
	Python::Register_Mem_Function<Foo, __LINE__>("getFloat", fooFn, METH_VARARGS, "Testing a member function");

	// TODO fix void functions
	std::function<void(Foo *, int)> fooFn2(&Foo::testVoid1);
	Python::Register_Mem_Function<Foo, __LINE__>("testVoid1", fooFn2, METH_VARARGS, "Testing a member function");

	std::function<void(Foo *)> fooFn3(&Foo::testVoid2);
	Python::Register_Mem_Function<Foo, __LINE__>("testVoid2", fooFn3, METH_VARARGS, "Testing a member function");

	std::function<int(Foo *)> fooFn4(&Foo::testVoid3);
	Python::Register_Mem_Function<Foo, __LINE__>("testVoid3", fooFn4, METH_VARARGS, "Testing a member function");
	return true;
}
